...
do
    for cc in $compilers; do
        ...
        for N in $N_list; do
            ...
            echo "$N"
            taskset -c 4 ./reduc ${N} 10 > data/${N}_${cc}_${i}_reduc.dat
        done
    done
    i=$((i+1))
done
...
for cc in $compilers; do
    ...
    out=data/compiler/${cc}_reduc.dat
    awk -F ';' "{print \$1 }" ${_file} |\
        tr -s "\n" ";" | tr -s " " " " |\
        sed -e '1 s/^[^;]*/N/' > ${out}
...
i=2
for f in $funcs; do
    out=data/func/${f}_reduc.dat
    cut -d ';' -f1 ${__file} > ${out}
    for cc in $compilers; do
        cut -d ';' -f${i} data/compiler/${cc}_reduc.dat | sed -e "1 s/^[^\n]*/${cc}/" | paste -d ";" ${out} - > ${tmpfile}
        cat ${tmpfile} > ${out} 
    done 
    i=$((i+1))
done
...
for func in $funcs; do
    ...
    for cc in $compilers; do
        out=data/flags/${func}/${cc}_reduc.dat
        echo ${header} > ${out}
        for N in $N_list; do
            line="${N}"
            iflag=1
            for flags in "${flags_list[@]}"; do
                file=data/${N}_${cc}_${iflag}_reduc.dat
                line="${line};"
                # echo ";" >> ${out}
